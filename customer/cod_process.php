...
if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    if(empty($address)){
        ...
        //insert query for cod order
    $insert_query= mysqli_query($conn, "INSERT INTO `orders`( `order_id`, `user`, `order_address`, `order_amount`, `coupon`, `payment_type`, `payment_id`,`payment_status`) VALUES ('$order_id','$user','$address','$order_amount','$coupon','$payment_mode','Cash On Delivery','Pending')");
    ...
    if($insert_query){
        ...
        if($order_item_insert_query){
            ...
            if($cart_empty_query){
                header("location:thanku.php?oid=$order_id&order_amount=$order_amount&opid=Cash on Delivery");
            }
        } 
    }    
}  
